...
最近的 Claude Code 二进制（2.1.148+）通过 `proper-lockfile` 在跨进程共享 `~/.claude/.oauth_refresh.lock`，持有时用 60 秒过期窗口并每 5 秒更新一次心跳（2.1.269 至 2.1.271 实测）。漏洞出在心跳停滞超过 60s 的持有者，或在无持有者时直接发现刷新令牌已过期的第二个客户端 —— 竞争仍然发生。
