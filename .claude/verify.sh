...
# exits 0. Measured on <linux-host>: dropping the whole `deploy-live` block —
...
#    macs exited 2 with "syntax error near unexpected token `;;'" while <linux-host>'s
...
#
# MATCH BOTH INVOCATION NAMES. The same file is reached under two basenames:
# directly as `bin/claude-via-proxy.mjs`, and through the npm-installed symlink
# `/opt/homebrew/bin/cache-fix-proxy -> .../claude-via-proxy.mjs`. A cmdline
# grep sees the name it was launched under, not the file it resolves to, so a
# pattern naming one shape is structurally blind to the other.
#
# Measured 2026-08-15, all three machines: <linux-host> and the personal Mac
# run `claude-via-proxy.mjs run-service`; the work Mac runs
# `cache-fix-proxy run-service` and matched NOTHING. Its run dir is absent too,
# so both discovery sources missed and proxy-live reported FAIL — about a proxy
# that was listening on 9901, carrying a real request in 0.14s, and whose
# holder had CACHE_FIX_PROXY_PORT=9901 sitting in the environment this loop
# reads. The cascade is the damage: deploy-live and ca-trust-live are gated on
# discovery, so the one check that proves the RUNNING process executes the
# DEPLOYED tree had never once run on that machine.
if [ -z "$ports" ]; then
  for h in $(pgrep -f '(claude-via-proxy\.mjs|cache-fix-proxy) run-service' 2>/dev/null); do
...
if [ -z "$ports" ]; then
  say proxy-live FAIL "no cachefix-<port> run dir under $tmp or /tmp, and no process matching 'claude-via-proxy.mjs|cache-fix-proxy run-service' names a port — if a proxy IS live here, it was launched under a third name and this pattern is what needs widening"
else
...
for port in ${ports:-}; do
  ...
  #
  #    ASK EVERY LISTENER, NOT THE FIRST. The port is held by several processes
  #    at once by design (fd handover): holder, gap-relay, server. Only some of
  #    them carry $D in their cmdline — the holder can be launched through npm's
  #    global bin, which is a symlink (/opt/homebrew/bin/cache-fix-proxy ->
  #    ../lib/node_modules/claude-code-cache-fix/bin/claude-via-proxy.mjs,
  #    resolving inside $D), and a cmdline shows the name it was launched under,
  #    never the file it resolves to.
  #
  #    Measured 2026-08-15 on the work Mac: lsof returns 8227 8263 8264, in that
  #    order, on every run. `head -1` therefore picked the holder every time,
  #    the holder's cmdline carried no occurrence of $D, and this check called
  #    our own live deploy "another install" — the one branch that reports OK
  #    while looking at nothing. It takes ca-trust-live with it, since that is
  #    gated below, so on that machine ca-trust-live had never run at all. Not a
  #    flaky ordering: deterministically the wrong process, deterministically.
  #
  #    Resolving each cmdline token with readlink -f also fixes it, and is NOT
  #    here: mutation-tested, removing it left every machine green, because the
  #    server is always spawned by explicit path and so always carries $D. A
  #    guard no test can kill is a guard the next reader deletes.
  lpids=$(lsof -nP -t -iTCP@127.0.0.1:"$port" -sTCP:LISTEN 2>/dev/null)
  [ -z "$lpids" ] && continue
  ours=no
  for lpid in $lpids; do
    lcmd=$(ps -o command= -p "$lpid" 2>/dev/null)
    case "$lcmd" in *"$D"*) ours=yes; break ;; esac
  done
  if [ "$ours" != yes ]; then
