...
- **OAuth-refresh lock now matches the client's real stale window, not `proper-lockfile`'s 10s library default (any release with `CACHE_FIX_OAUTH_REFRESH=on`).** `proxy/oauth/refresher.mjs`'s `lockOpts()` acquired `.oauth_refresh.lock` with `stale: 10000` — never a value the client actually passes, just `proper-lockfile`'s own default, misread as the client's stale-break window when the directive was written from the original binary inspection. Measured 2026-09-14 across Claude Code 2.1.269, 2.1.270 and live 2.1.271: the client holds the lock with `{realpath: false, stale: 60000, update: 5000}`. So a client whose lock heartbeat merely stalled past 10s (well inside the client's own 60s tolerance) would still believe itself the lock owner while the proxy read the same lock as stale, took it, and POSTed the same refresh token — the double-refresh, family-revoking race this subsystem exists to prevent, now caused by the proxy itself. `lockOpts()` and `CLIENT_STALE_WINDOW_MS` now match the client's real `{stale: 60000, update: 5000}`; `CACHE_FIX_OAUTH_POST_TIMEOUT_MS` (still 8000 ms default) is unchanged and stays well under the corrected window. See [`docs/directives/proxy-owned-oauth-refresh.md`](docs/directives/proxy-owned-oauth-refresh.md) §2a.
