...
- **`claude-opus-5-5` is now priced, tracked for divergence, and no longer billed as `claude-opus-5`.** The model tables (`tools/rates.json`, `tools/update-rates.mjs`, `proxy/model-families.mjs`) had no entry for it, so `tools/cost-report.mjs` fell back to its longest-matching neighbor — `claude-opus-5`'s rates, 25% too high on input/output and off by more than 2x on the cache-read discount the pricing page documents for this model — and `cache-telemetry.mjs`'s served-model divergence check classified it as `"unknown"` rather than `"opus"`.
