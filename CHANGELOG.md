...
- **`claude-opus-5-5` is now priced, tracked for divergence, and no longer billed as `claude-opus-5`.** The model tables (`tools/rates.json`, `tools/update-rates.mjs`, `proxy/model-families.mjs`) had no entry for it, so `tools/cost-report.mjs`'s `lookupRates()` matched the first `rates.json` key in file order where either id prefixed the other — `claude-opus-5`'s rates, 25% too high on input/output and off by more than 2x on the cache-read discount the pricing page documents for this model — and bare `claude-opus-5` / `claude-sonnet-5` read as `"unknown"` in `proxy/model-families.mjs`, so a request served by a different family only latched `model_divergence_sticky` on the third divergent turn instead of the first.
...
- **`proxy/oauth/refresher.mjs` now acquires `.oauth_refresh.lock` with the client's real `{stale: 60000, update: 5000}` instead of `proper-lockfile`'s 10s default; `CACHE_FIX_OAUTH_POST_TIMEOUT_MS` is unchanged.** The proxy no longer breaks a lock a client still holds after a 10s heartbeat stall, so it cannot become the second refresher. See [`docs/directives/proxy-owned-oauth-refresh.md`](docs/directives/proxy-owned-oauth-refresh.md) §2a.
