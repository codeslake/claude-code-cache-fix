...
- The client **does** have a cross-process lock: `lockfilePath = join(<dir>, ".oauth_refresh.lock")`, where `<dir>` resolves to the **shared** config dir (`CLAUDE_SECURESTORAGE_CONFIG_DIR ?? ~/.claude`), with `stale: 10000` ms. (Measured 2026-09-14 on 2.1.269-2.1.271: the client actually passes `stale: 60000, update: 5000`; the `10000` read here off 2.1.148 was proper-lockfile's own library default, not a value the client set explicitly. See §2a.)
- So the lock is a `~/.claude/.oauth_refresh.lock`, shared across the fleet — but it has a **hole**: a holder whose heartbeat stalls past the stale window (the holding process hangs or dies, so `proper-lockfile`'s own `update` timer stops touching the lock's mtime). A second client's stale-check then passes, it proceeds **without** the lock, and both POST the same refresh token to the token endpoint → family revoked. (A holder that is merely slow but alive keeps its heartbeat running, so the lock never goes stale under it — that path is not the hole.)
...
This is the failure mode the whole directive exists to eliminate, so it is contracted explicitly, not assumed. The proxy's own heartbeat (`update: 5000`) keeps its held lock from going stale on its own, so a merely-slow POST does not by itself let a waking client steal the lock — the real risk is leaving the refresh **outcome** UNKNOWN (and the proxy's own tick loop blocked) for longer than necessary, and the residual case where the proxy process itself hangs or dies (its heartbeat stops with it, and so would the deadline abort below — the deadline is defense in depth for a live-but-slow POST, not a substitute for the client's own stale-break covering a truly dead proxy).

Contract:
- The refresh POST has a **hard deadline of `CACHE_FIX_OAUTH_POST_TIMEOUT_MS`, default 8000 ms** — strictly below the client's 60 000 ms stale threshold, with ample margin for the lock acquire + disk write inside the same held-lock window. This bounds how long the refresh's outcome stays UNKNOWN and how long the proxy blocks its own tick loop; the implementation must keep the non-POST steps fast (sub-second) so the POST owns essentially the whole budget.
...
- Because the proxy aborts at 8 s, well inside the client's 60 s stale window, a live proxy always gives up and backs off long before any staleness question could even arise; the ordering guarantees that if the proxy ever loses the timing race, it loses by *not POSTing again*, not by POSTing concurrently.
