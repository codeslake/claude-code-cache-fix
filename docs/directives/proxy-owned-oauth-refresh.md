...
- So the lock is a `~/.claude/.oauth_refresh.lock`, shared across the fleet — but it has a **hole**: a holder whose heartbeat stalls past the stale window (the holding process hangs or dies, so `proper-lockfile`'s own `update` timer stops touching the lock's mtime), or a second client that simply finds the refresh token already expired with no holder at all. Either way a second client's stale-check passes, it proceeds **without** the lock, and both POST the same refresh token to the token endpoint → family revoked.
